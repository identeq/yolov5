...
from yolov5.utils.general import (LOGGER, ROOT, Profile, check_requirements, check_suffix, check_version, colorstr,
                                  increment_path, make_divisible, non_max_suppression, scale_coords, xywh2xyxy,
                                  xyxy2xywh,
                                  yaml_load)
...
class AutoShape(nn.Module):
    ...
    def forward(self, ims, size=640, augment=False, profile=False, conf=0.25, iou=0.45, agnostic=False,
                multi_label=False, classes=None, max_det=1000):
...
class AutoShapeDefault(nn.Module):
    # YOLOv5 input-robust model wrapper for passing cv2/np/PIL/torch inputs. Includes preprocessing, inference and NMS
    conf = 0.25  # NMS confidence threshold
    iou = 0.45  # NMS IoU threshold
    agnostic = False  # NMS class-agnostic
